...
class SolanaWalletTracker:
    ...
    def get_wallet_info(self, wallet_address):
        ...
        try:
            response = requests.get(
                f"https://api.solscan.io/v1/account/overview/{wallet_address}",
                headers={
    ...
    def get_top_pumpfun_creators(self, limit=20):
        ...
        try:
            ...
            for pair in tokens:
                ...
                wallet_stats[creator]["total_volume"] += volume
                wallet_stats[creator]["token_count"] += 1
                wallet_stats[creator]["tokens"].append({
                    "name": pair.get("baseToken", {}).get("name", ""),
                    "address": base_mint,
                    "mcap": mcap,
                    "volume": volume,
                })
                wallet_stats[creator]["total_mcap"] += mcap
                wallet_stats[creator]["total_liquidity"] += liquidity
    ...
    def print_top_tokens(self):
        ...
        for i, pair in enumerate(tokens, 1):
            ...
            print(f"   Address: {self.format_wallet_address(base_mint)}")
    ...
    def run(self):
        ...
        print("\n" + "=" * 80)
        print("💡 TIP: Wallet addresses are formatted with [COPY] prefix for easy copying")
        print("=" * 80)
